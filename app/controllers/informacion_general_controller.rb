...
class InformacionGeneralController < ApplicationController
  ...
  def editar_grupos
    ...
    ActiveRecord::Base.transaction do
      begin
        gp = GrupoPequenio.lock.find(form[:idgroup]).update({:var_grupopequenio_nombre => form[:nombre],
          :int_grupopequenio_tipo => form[:tipo_grupo],
          :lugar => Lugar.find(form[:lugar]) ,
          :grupo_principal => GrupoPrincipal.find(form[:grupo_principal]),
          :dat_grupopequenio_fechaInicio => form[:fecha_grupo],
        ...
        ListaMiembroGp.lock.where({:grupo_pequenio_id => form[:idgroup] }).update({:var_listamiembrogp_estado => "0"})
        if tabla != nil
          tabla.each do |y|
            x = y.last
            #lm = ListaMiembroGp.create!({:grupo_pequenio=> gp,
  ...
  def lista_personas_grupo
    persona = ActiveRecord::Base.connection.execute("SELECT * FROM sp_get_personas_listamiembrogp("+params[:id]+")") 
